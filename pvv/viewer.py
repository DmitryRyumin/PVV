...
# Персональные
from pvv.modules.filem.json import Json  # Работа с JSON
...
class Messages(Json):
    ...
    def __init__(self):
        ...
        self._data_not_received = self._('[{}{}{}] Данные не получены ...')
        self._program_close = self._('[{}] Программа закрыта ...')
...
class Viewer(Messages):
    ...
    def __draw(self):
        ...
        if self._idle_function is not None:
            if self._idle_function(out = self._out) is -1:
                return None

            if self._idle_function(out = self._out) is False:
                # Печатать процесс выполнения
                if self._out is True:
                    stdout = self._data_not_received.format(
    ...
    def start(self):
        ...
        GLUT.glutSpecialFunc(self.__keyboard)
